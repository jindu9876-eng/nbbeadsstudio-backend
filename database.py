    ...
    try:
        ...
        if admin_count == 0:
            ...
            for i in range(1, 11):
                user = User(
                    name=f"User {i}",
                    email=f"user{i}@nbbeadsstudio.com",
            ...
            logger.info("Default MongoDB seed data populated successfully.")
    except Exception as e:
        logger.error(f"Error seeding MongoDB data: {e}", exc_info=True)
